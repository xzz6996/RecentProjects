...
//				topAccountId:this.props.location.state.topAccountId,
				taskId:this.props.location.state.taskId,
				roleId:this.props.location.state.roleId,
				topRoleId:this.props.location.state.topRoleId,
...
class OccupationalInformation extends React.Component {
	...
	componentDidMount(e) {
		// 组件装载完成以后声明一个自定义事件
		  emitter.addListener('loansave', (valuess,obj) => {
		  	  if( valuess == 1 ){
		  		    this.props.form.validateFields((err, valuess) => {
			        		if (!err) {
											emitter.emit( 'loansaveone',valuess ,obj );
			      			}
			    		});
		  	  }else{
		  	  		emitter.emit( 'loansaveone','false' );
		  	  }
		  });
	}
	...
	componentDidMount(e) {
		...
		emitter.addListener('loansave', (valuess,obj) => {
				if( valuess == 1 ){
					  this.props.form.validateFields((err, valuess) => {
					      if (!err) {
									  emitter.emit( 'loansavetwo',valuess,obj );
					      }
		    		});
				}else{
						emitter.emit( 'loansavetwo', 'false' );
				}
	  });
	}
	        	...
	        	<Row gutter={24}>
					...
					<Col xl={8} xxl={8}>
				 	...
				 	<Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="身份证号"  >
						    {getFieldDecorator('spouseCardId',{initialValue : this.props.info.spouseCardId})(
				 	...
				 	<Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="社保公积金"  >
						    {getFieldDecorator('spouseAccumulationFundLines',{initialValue : this.props.info.spouseAccumulationFundLines })(
				 	...
				 	<Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="婚姻"  >
						   {getFieldDecorator('spouseMarriage' , {
				    ...
				    <Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="学历"  >
						    {getFieldDecorator('spouseEducationalBackground',{initialValue : this.props.info.spouseEducationalBackground})(
				    ...
				    <Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="户籍住址"  >
						    {getFieldDecorator('spouseNativePlace',{initialValue : this.props.info.spouseNativePlace})(
				     ...
				     <Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="移动电话"  >
						    {getFieldDecorator('spousePhone',{initialValue : this.props.info.spousePhone})(
				     ...
				     <Col xxl={8} xl={8}>
					    <FormItem {...formItemLayout} label="居住地址"  >
						    {getFieldDecorator('spouseOftenPlace',{initialValue : this.props.info.spouseOftenPlace })(
...
class LoanAssessment extends React.Component {
	...
	componentDidMount() {
		emitter.addListener('loansave', (valuess) => {

			emitter.emit( 'loansavethree',this.state.data );
...
class LoanToExamine extends React.Component {
	...
	componentDidMount(){
			emitter.addListener('loansaveone', (loansaveone,obj) => {
					if( loansaveone != 'false' ){
							obj.workUnits = loansaveone.workUnit;
							obj.unitAddress = loansaveone.loansaveone;
							obj.industry = loansaveone.industry;
							obj.department = loansaveone.department;
							obj.jobLevel = loansaveone.jobLevel;
							obj.monthlySalary = loansaveone.monthlySalary;
							obj.otherIncome = loansaveone.otherIncome;
							obj.otherIncomeSources = loansaveone.otherIncomeSources;
							obj.unitPhone = loansaveone.unitPhone;
					}
					emitter.addListener('loansavetwo', (loansavetwo,obj) => {
							if( loansavetwo != 'false' ){
										obj.spouseName = loansavetwo.spouseName;
										obj.spouseAccumulationFundLines = loansavetwo.spouseAccumulationFundLines;
										obj.spouseCardId = loansavetwo.spouseCardId;
										obj.spouseEducationalBackground = loansavetwo.spouseCardId;
										obj.spouseMarriage = loansavetwo.spouseMarriage;
										obj.spouseNativePlace = loansavetwo.spouseNativePlace;
										obj.spouseOftenPlace = loansavetwo.spouseOftenPlace;
										obj.spousePhone = loansavetwo.spousePhone;
										obj.spouseSex = loansavetwo.spouseSex;
							}
							emitter.addListener('loansavethree', (loansavethree,obj) => {
	...
	handleSubmit(e) {
      ...
      if (!err) {
        let obj = {};
				...
				obj.roleType = this.props.orderid.status;
				obj.status = values.status;
				obj.accountId = this.state.accountId;
//				obj.topAccountId = this.props.orderid.topAccountId;
				obj.taskId = this.props.orderid.taskId;
				obj.roleId = this.props.orderid.roleId;
				obj.topRoleId = this.props.orderid.topRoleId;
				obj.instanceId = this.props.orderid.instanceId;
				obj.remark = values.describtion;

				if ( this.props.orderid.status == 1 ) {
					emitter.emit('loansave', 1,obj);
				}else{
